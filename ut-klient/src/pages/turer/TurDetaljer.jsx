  ...
  return (
    <PageWrapper>
      <div className="Turer TurDetaljerSide">
        <button className="TilbakeKnapp" onClick={() => navigate("/turer")}>
        ...
        {!loading && !error && tur && (
          <div className="tur-detaljer">
            ...
            {tur.bilder && tur.bilder.length > 0 && <EmblaCarousel slides={tur.bilder} options={{ loop: true }} />}

            <div className="tur-info-og-anmeldelse">
              <div className="tur-info">
                {tur.hoyeste_punkt_moh && (
                  <p>
                    <strong>Høyeste punktet på turen:</strong> {tur.hoyeste_punkt_moh} moh
                  </p>
                )}

                {tur.stier[0]?.kommune_navn && (
                  <p>
                    <strong>{t("felles.kommune")}:</strong> {tur.stier[0]?.kommune_navn}
                  </p>
                )}

                {tur.stier[0]?.fylke_navn && (
                  <p>
                    <strong>{t("felles.fylke")}:</strong> {tur.stier[0]?.fylke_navn}
                  </p>
                )}
                {tur.vanskelighetsgrad && (
                  <p>
                    <strong>{t("felles.vanskelighetsgrad")}:</strong> {t(`enums.vanskelighetsgrad.${tur.vanskelighetsgrad}`)}
                  </p>
                )}

                {tur.turtype && (
                  <p>
                    <strong>{t("tur.turtype")}:</strong> {t(`enums.turtype.${tur.turtype}`)}
                  </p>
                )}

                {tur.varighet && (
                  <p>
                    <strong>{t("tur.varighet")}:</strong> {t(`enums.varighet.${tur.varighet}`)}
                  </p>
                )}

                {tur.tur_beskrivelse && (
                  <p>
                    <strong>{t("tur.beskrivelse")}:</strong> {tur.tur_beskrivelse}
                  </p>
                )}

                {tur.lengde && (
                  <p>
                    <strong>Rutelengde :</strong> {tur.rute_lengde.toFixed(2)} KM
                  </p>
                )}
              </div>
              ...
              {kanSkriveAnmeldelse && (
                <div className="tur-anmeldelse-side">
                  <AnmeldelseSkjema
                    onSend={(data) => leggTilTurAnmeldelse(turId, data, bruker?.bruker_navn, bruker?.bruker_id)}
                    loading={loading}
                  />
                </div>
              )}

...
export default function Varsler() {
  ...
  const { varsler, valgtVarsel, setValgtVarsel, loading, error, hentVarsel, merkSomLest, slettVarsel, behandleOppgave } = useVarsler({
    token,
    autoPoll: erAutentisert,
  });
    ...
    <PageWrapper>
      <div className="varsler-side">
        <div className="varsler-layout">
          <VarselListe varsler={varsler} valgtId={valgtId} onVelg={handleVelgVarsel} />
          <div className="varsler-innhold">
            <VarselDetaljer varsel={valgtVarsel} loading={loading} error={error} onBehandle={behandleOppgave} onSlett={slettVarsel} />
          </div>
        </div>
      </div>
    </PageWrapper>

  ...
  return (
    <PageWrapper>
      <div className="kart-knapp-container">
        <div>{!lagret && <GpxParser onKoordinaterLastet={handleGpxKoordinater} />}</div>
        <button onClick={open}>{lagret ? "Vis Tur" : "Lag Tur"}</button>
        {lagret && (
          <>
            <span className="koordinater-lagret-melding">✓ Koordinater hentet</span>
            <span>Rutelengde: {totalRuteLengde.toFixed(3)}KM</span>
          </>
        )}

        {høydeMeter && <span>Høyde: {høydeMeter}</span>}

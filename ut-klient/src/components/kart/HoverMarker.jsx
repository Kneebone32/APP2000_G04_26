...
export function HoverMarker({ objekt, children, hoverFaktor = 1, onKlikk, ikon }) {
  ...
  const { normalIkon, hoverIkon } = useMemo(() => {
    ...
    const hoverStørrelse = [normalStørrelse[0] * hoverFaktor, normalStørrelse[1] * hoverFaktor];
    const hoverAnchor = [normalAnchor[0] * hoverFaktor, normalAnchor[1] * hoverFaktor];

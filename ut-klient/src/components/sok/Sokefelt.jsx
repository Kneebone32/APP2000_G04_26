/* Skrevet av Kristoffer med hjelp av AI */
...
export default function Sokefelt({ onNavigate }) {
  ...
  const visDropdown = open && q.trim().length >= 2;
    ...
    <div className="sokefelt" ref={containerRef}>
      <div className="sokefelt-input-wrapper">
        ...
        <input
          ...
          placeholder={t("sokefelt.placeholder")}
          value={q}
          onChange={(e) => {
            setQ(e.target.value);
            setOpen(true);
          ...
          aria-label={t("sokefelt.aria_label")}
        />
      </div>

      {visDropdown && (
        <div className="sokefelt-dropdown" role="listbox">
          {loading && <div className="sokefelt-melding">{t("sokefelt.laster")}</div>}
          {error && <div className="sokefelt-melding sokefelt-feil">{error}</div>}
          {!loading && !error && !harTreff && <div className="sokefelt-melding">{t("sokefelt.ingen_treff")}</div>}

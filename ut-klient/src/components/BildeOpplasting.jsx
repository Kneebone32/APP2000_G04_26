        ...
        <div style={{ marginTop: "10px" }}>
          ...
          {bildeUrl?.map((url, index) => (
            <div key={index} style={{ display: "inline-block", position: "relative", marginRight: "10px" }}>
              <img className="Bilde" src={`${url}?w=200&h=200&fit=fit`} alt={`Preview ${index + 1}`} />
              <button className="fjern-bilde" type="button" onClick={() => setBildeUrl(bildeUrl.filter((_, i) => i !== index))}>
                ✕
              </button>
            </div>
          ))}

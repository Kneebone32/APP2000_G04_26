    ...
    <>
      ...
      <div className="multi-select-filter">
        ...
        <select className="filter-dropdown" onChange={handleSelectChange}>
          ...
          {alleValg.map((valg) => (
            <option key={valg.navn} value={valg.id} disabled={valgteFasiliteter.some((f) => f.id === valg.id)}>
              {valg.navn}
            </option>
          ))}
        </select>
        ...
        <div className="tag-container">
          {valgteFasiliteter.map((fasilitet) => (
            <span key={fasilitet.id} className="tag-pill">
              {fasilitet.navn}
              <button type="button" className="tag-fjern-btn" onClick={() => onToggle(fasilitet)}>
                ×
              </button>
            </span>
          ))}

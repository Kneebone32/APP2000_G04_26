...
export function useArtikkel({ autoFetch = false, token = null } = {}) {
  ...
  const opprettArtikkel = useCallback(
    async ({ artikkel_slug, artikkel_tittel, artikkel_innhold }) => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch(`${import.meta.env.VITE_API_URL}/artikkel`, {
  ...
  const redigerArtikkel = useCallback(
    async (id, { artikkel_tittel, artikkel_innhold, artikkel_slug }) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const slettArtikkel = useCallback(
    async (id) => {
      try {
        setLoading(true);
        setError(null);

...
export function useMeldinger({ token, pollIntervall = 5000, autoPoll = false, autoFetch = false, valgtSamtaleId = null } = {}) {
  ...
  const hentEllerOpprettDirekte = useCallback(
    async (annenBrukerId) => {
      if (!token) return;
      try {
        setError(null);
  ...
  const opprettSamtale = useCallback(
    async (brukerIds, samtalenavn = null) => {
      if (!token) return;
      try {
        setError(null);
  ...
  const forlatSamtale = useCallback(
    async (samtaleId) => {
      if (!token) return;
      try {
        setError(null);
  ...
  const markerSamtaleLest = useCallback(
    async (samtaleId) => {
      if (!token) return;
      try {
  ...
  const sendMelding = useCallback(
    async (samtaleId, meldingTekst, bildeUrl = null) => {
      if (!token) return;
      try {
        setError(null);

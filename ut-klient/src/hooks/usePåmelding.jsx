...
export function usePåmelding({ token, aktivitetId = null } = {}) {
  ...
  const hentEksisterendePåmelding = useCallback(
    async (id) => {
      if (!token) return;
      try {
  ...
  const hentMinPåmelding = useCallback(
    async (id) => {
      if (!token) return;
      try {
        setLoading(true);
  ...
  const registrerBildesamtykke = useCallback(
    async (datoId, samtykke) => {
      if (!token) return;
  ...
  const låsPåmelding = useCallback(
    async (aktivitetId, erLast) => {
      if (!token) return;
      try {
        setLoading(true);
        setError(null);
  ...
  const velgFastDato = useCallback(
    async (aktivitetDatoId) => {
      if (!token) return;
      try {
        setLoading(true);
        setError(null);

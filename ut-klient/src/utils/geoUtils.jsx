...
export const hentHøydeMåling = async (lat, lon) => {
  try {
    const response = await fetch(`https://ws.geonorge.no/hoydedata/v1/punkt?nord=${lat}&ost=${lon}&koordsys=4326`);
...
export const hentHøydeProfil = async (koords) => {
  ...
  const utvalgteKoords = Array.from({ length: antall }, (_, index) => koords[Math.round((index * (koords.length - 1)) / (antall - 1))]);
...
export const byggPunkterMedMoh = async (koords) => {
  ...
  const antall = Math.min(50, koords.length);
  const sampledIndices = Array.from({ length: antall }, (_, i) => Math.round((i * (koords.length - 1)) / (antall - 1)));
...
export const hentKommuneData = async (lat, lon) => {
  try {
    const response = await fetch(`https://ws.geonorge.no/kommuneinfo/v1/punkt?nord=${lat}&ost=${lon}&koordsys=4326`);
